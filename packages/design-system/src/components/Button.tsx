...
const BUTTON_VARIANTS = {
  primary: 'bg-pink-400 text-white border border-pink-400',
  secondary: 'bg-pink-100 text-pink-400 border border-pink-100',
  outline: 'bg-white text-gray-800 border border-gray-300 enabled:hover:bg-gray-50',
...
function LoadingButton({ className }: { className: string }) {
  return (
    <>
      <span
        className={`mr-4 inline-block h-16 w-16 animate-spin rounded-full border-2 border-current border-r-transparent ${className}`}
      />
...
export default function Button({
  ...
  const loaderClassNames = twMerge('opacity-50', LOADER_VARIANTS[variant]);
